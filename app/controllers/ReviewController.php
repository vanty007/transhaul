...
class ReviewController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$fields = array('id', 	'user_id', 	'rating', 	'comment', 	'pickup_request_id', 	'created_at');
		...
		if(!empty($this->search)){
			...
			$db->orWhere('pickup_request_id',"%$text%",'LIKE');
		...
		$records = $db->get('review', $limit, $fields);
		$data = new stdClass;
	...
	function import_data(){
		if(!empty($_FILES['file'])){
			...
			if(!empty($file_path)){
				$db = $this->GetModel();
				if($ext == 'csv'){
					$options = array('table' => 'review', 'fields' => '', 'delimiter' => ',', 'quote' => '"');
					$data = $db->loadCsvData( $file_path , $options , false );
				}
				else{
					$data = $db->loadJsonData( $file_path, 'review' , false );
				}
				if($db->getLastError()){
	...
	function view( $rec_id = null , $value = null){
		...
		$record = $db->getOne( 'review', $fields );
		if(!empty($record)){
			render_json($record);
		}
		else{
	...
	function add(){
		if(is_post_request()){
			...
			$rules_array = array(
				...
				'comment' => 'required',
				'pickup_request_id' => 'required|numeric',
			);
			...
			$db = $this->GetModel();
			$rec_id = $db->insert('review',$modeldata);
			if(!empty($rec_id)){
				render_json($rec_id);
			}
			else{
	...
	function edit($rec_id=null){
		...
		if(is_post_request()){
			$modeldata=transform_request_data($_POST);
			$db->where('id' , $rec_id);
			$bool = $db->update('review',$modeldata);
			if($bool){
				render_json($rec_id);
			}
			else{
			...
			$fields=array('id','user_id','rating','comment','pickup_request_id');
			$db->where('id' , $rec_id);
			$data = $db->getOne('review',$fields);
			if(!empty($data)){
				render_json($data);
			}
			else{
